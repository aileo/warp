...
WORKDIR /opt/warp
RUN apt-get update && \
    apt-get install -y --no-install-recommends wget mailcap build-essential libpq-dev libpcre2-8-0
...
RUN apt-get update && \
    apt-get install -y --no-install-recommends libpq5 mailcap libpcre2-8-0 && \
